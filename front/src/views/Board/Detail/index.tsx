...
import { getBoardResquest, getCommentListRequest, getFavoriteListRequest, increaseViewCountRequest } from 'apis';
...
export default function BoardDetail() {
  ...
  const BoardDetailTop = () => {
    ...
    const getBoardResponse = (responseBody: GetBoardResponseDto | ResponseDto | null) => {
      ...
      console.log('게시물 데이터:', responseBody);
      if (code === 'NB') alert('존재하지 않는 게시물입니다.');
      if (code === "DBE") alert('데이터베이스 오류입니다.');
      if (code !== 'SU') {
        navigator(MAIN_PATH());
            ...
            {loginUser !== null &&
              <div className='board-detail-bottom-comment-input-box'>
                <div className='board-detail-bottom-comment-input-container'>
                  <textarea ref={commentRef} className='board-detail-bottom-comment-textare' placeholder='댓글을 작성해주세요.' value={comment} onChange={onCommentChangeHandler} />
                  <div className='board-detail-bottom-comment-button-box'>
                    <div className={comment === '' ? 'disable-button' : 'black-button'} onClick={onCommentSubmitButtonClickHandler}> {'댓글쓰기'}</div>
                  </div>
                </div>
              </div>
            }
  ...
  // effect: 게시물 번호 path variable 바뀔때마다 게시물 조회수 증가
  let effectFlag = true;
  useEffect(() => {
    if (!boardNumber) return;
    if (effectFlag) {
